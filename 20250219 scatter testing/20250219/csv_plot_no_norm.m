...
%filename_array = {'short w mag.csv'};
%filename_array = {'long barrel.csv'};
%filename_array = {'short w_o mag.csv'};
...
title('Comparing Barrel Spread (natural)')
%title('short w_o mag Barrel Spread (natural)')

xlabel('depth spread [in]')
ylabel('horizontal spread [in]')
zlabel('vertical spread [in]')

legend({'Compound Misfire'  '' str 'Short Barrel with Mag' '' str1})% filename_array{3} '' str2 filename_array{4} '' str3 })

%legend({filename_array{2} '' str1 filename_array{3} '' str2})

%legend({filename_array{1} '' str})
%legend({filename_array{2} '' str1})
%legend({filename_array{3} '' str2})
...
function str1= plot_me(filename,file_n,color1,color2)
    ...
    scatter(x,y, 'MarkerFaceColor',color1); hold on;
    grid on 
    ...
    plot(xc, yc,'--', 'Color',color2,'LineWidth',2);  
