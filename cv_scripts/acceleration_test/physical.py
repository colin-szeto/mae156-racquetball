...
# Reference circle setup
reference_center = (400, 300)  # (x, y) in pixels
...
while True:
    ...
    cv2.line(frame, (x - r, y), (x + r, y), (200, 200, 0), 1)  # Horizontal
    cv2.line(frame, (x, y - r), (x, y + r), (200, 200, 0), 1)  # Vertical
    ...
    if closest_sharpie_contour is not None and closest_sharpie_centroid:
        ...
        # Draw motion data
        cv2.putText(frame, f"Angle: {angle_deg:.2f}°", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        if angular_velocity is not None:
            cv2.putText(frame, f"Angular Vel: {math.degrees(angular_velocity):.2f}°/s", (10, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 200, 255), 2)
        if angular_acceleration is not None:
            cv2.putText(frame, f"Angular Accel: {math.degrees(angular_acceleration):.2f}°/s²", (10, 90),
    ...
    # Show result
    cv2.imshow("Sharpie Rotation Tracking", frame)
