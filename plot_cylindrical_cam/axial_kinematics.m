...
deg_lim = 120;
subplot(4,1,1)
% plot(theta_deg, (theta_rad / omega_const), 'm','LineWidth',2);
plot(theta_deg, s, 'm','LineWidth',2);
ylabel({'Index Ram' ; 'Displacement'}); grid on;
xlim([0 deg_lim]);

%subplot(5,1,2)
%plot(theta_deg, theta_deg, 'b','LineWidth',2);
%ylabel('\theta (deg)'); grid on;
%xlim([0 deg_lim]);

subplot(4,1,2)
plot(theta_deg, v, 'g','LineWidth',2);
ylabel('\omega (mm/rad)'); grid on;
xlim([0 deg_lim]);

subplot(4,1,3)
plot(theta_deg, a, 'r','LineWidth',2);
ylabel('\alpha (mm/rad^2)'); grid on;
xlim([0 deg_lim]);

subplot(4,1,4)
plot(theta_deg, j, 'k','LineWidth',2);
ylabel('Jerk'); xlabel('\theta (deg)'); grid on;
xlim([0 deg_lim]);
